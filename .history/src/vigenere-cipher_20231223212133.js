...
class VigenereCipheringMachine {
  ...
  cipher(plainChar, keyChar) {
    ...
    const cipherIndex = this.cipherTable.indexOf(plainChar) + this.cipherTable.indexOf(keyChar);
    const cipherChar = this.cipherTable[cipherIndex % this.cipherTable.length];
    return String.fromCharCode(base + (cipherIndex / this.cipherTable.length) + (cipherChar - base) * 26);
  }
}
